...
\pagestyle{fancy}\fancyhf{}\fancyhead[L]{\small NYU Sociology / Research demonstration}\fancyhead[R]{\small V1}\fancyfoot[C]{\small\thepage}
...
\textbf{Substitution}: a person who would otherwise approach a friend instead uses a chatbot. This initial version deliberately contains no pathway from chatbot interaction back to human outreach.
...
\begin{align*}
 ...
 H_{it}&=H^0_{it}(1-A_{it})+0.
\end{align*}
There is at most one human request per need. The sensitivity check varies $s$ from 0 to 1 in steps of 0.2. The absence of bridging is a modeling restriction, not an empirical finding.
...
The no-chatbot scenario averages 54.6 human requests per 100 needs; substitution averages 28.6. The decline is expected: diversion removes baseline requests and no rule adds them back.

For a person with baseline request probability $h$, the expected request rate per need is
\[
 \mathbb{E}[H\mid\text{need}]=h(1-as).
\]
With positive access and diversion, a fall in requests follows directly. Increasing simulation precision cannot turn this imposed mechanism into an empirical estimate.
